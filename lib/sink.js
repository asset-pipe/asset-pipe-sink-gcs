...
module.exports = class SinkGCS extends EventEmitter {
    ...
    async get(fileName) {
        ...
        try {
            ...
            throw Boom.serverUnavailable(
                `File "${fileName}" exists, however multiple attempts to download the file from google cloud storage has failed.`,
                {
                    file: fileName,
    ...
    async set(fileName, fileContent) {
        ...
        try {
            ...
            throw Boom.serverUnavailable(
                `Unable to save file "${fileName}" to google cloud storage. 3 attempts failed.`,
                {
                    file: fileName,
                    description: err.message,
                }
